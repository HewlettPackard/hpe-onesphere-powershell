...
begin
  ...
  Log('Added ' + title + ' to product download list.');
...
begin
  Log(Format('Path is [%s], Exists = %d', [product.File, Integer(FileExists(product.File))]));
    ...
    //MsgBox('Second IF statement', mbInformation, MB_OK);
    Log(Format('Executing: "wusa.exe %s %s"', [product.File, product.Parameters]));
		Result := Exec('wusa.exe', product.File + ' ' + product.Parameters, '', SW_SHOWNORMAL, ewWaitUntilTerminated, ResultCode);
	end else begin
		Result := ShellExec('', product.File, product.Parameters, '', SW_SHOWNORMAL, ewWaitUntilTerminated, ResultCode);
	end;
  Log(Format('Install ResultCode: %d [%s]', [ResultCode, SysErrorMessage(ResultCode)]));
end;
...
begin
  ...
  MemoDirInfo := WizardForm.DirEdit.Text;
	s := s + 'Installation Directory:' + NewLine + '      ' + MemoDirInfo
  //MemoGroupInfo
